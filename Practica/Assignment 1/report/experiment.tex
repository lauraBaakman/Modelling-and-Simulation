...
\label{ss:fixed}
	...
	Considering these figures we see that the progression of both $p$ and $x$ happens according to a fixed pattern with a seemingly periodic shift. Examining \cref{fig:experiment:dimension:1:t} we see that in the map, there are 

	% Taking into account the values of $p$ one finds that the bottom left and upper right of \cref{fig:experiment:dimension:1:p} are associated with respectively the lower and the upper closed curve in {fig:experiment:dimension:1}. From this we can conclude the map first switches between generating some $p$-values for the upper and bottom closed curves in \cref{fig:experiment:dimension:1}. 
