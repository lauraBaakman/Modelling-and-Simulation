...
The parameter $K$ defines how hard the system is driven, that is why for $K = 0$ we would expect to have no perturbations. This case is more extensively discussed in \cref{ss:b:trivial}. As the value of $K$ increases we expect that the chaotic regions grow and eventually start to dominate the phase space \cite{finn200Cahaotic}. The effect of varying values of $K$ is discussed in \cref{ss:b:nontrivial}. Finally \cref{ss:b:kam} discusses KAM theory and how it relates to the Chirikov map. 
...
\begin{subequations}\label{eq:chirikovK0}
	...
	\end{align}
\end{subequations}	
Consequently the values of $p_n$ are constant for $n > 0$, since if $p_0 = 1$, then $p_1$ becomes zero as a consequence of the modulo operator in \eqref{eq:chirikov0:p}. Additionally without the modulo in \eqref{eq:chirikov0:x}, $x_n$ would increase linearly, but due to this operator $x_n$ becomes periodic. 
...
\Cref{fig:experiment:fancy_k} shows the influence of variable $K$. As introduced $K$ determines the drive of the system resulting in more chaotic behaviour with a higher $K$. The images in \cref{fig:experiment:fancy_k} were created by including 100 Chirikov maps with randomly chosen values for $x_0$ and $p_0$. The colour for each map was determined by its initialisation. The mapping from the colours to the initial values are shown in \cref{fig:experiment:fancy_k:colormap}. Note that because of the choice to colour the maps this way, we can see that up until $K = 1$ the colours stay in the same order, as opposed to $K = 2$ to $K = 4$ were this order is completely gone.

We consider a phase space to be more chaotic when there are more or bigger filled areas in the images. As can be expected from the discussion in our previous section the map created with $K = 0$ shows only horizontal (constant $p_n$) lines. Interesting is the difference in line types i.e. there are filled and dotted lines, indicating different periods for $x_n$. \todo[inline]{Ja... ofzo, denk ik.}

We found that by increasing $K$ with steps of $0.2$ up until one, (\cref{fig:experiment:fancy_k:2} - \ref{fig:experiment:fancy_k:10}), and then with steps of 1 (\cref{fig:experiment:fancy_k:20} - \ref{fig:experiment:fancy_k:40}) until a $K$ of four, clearly shows the influence of $K$. At $K = 0.2$ we see that the some of the horizontal lines are replaced by closed curves in the upper and lower part of the image, see \cref{fig:experiment:fancy_k:2}. Because the colours in the image map to initial values we see that the curves that appear in the upper and lower part were formed from approximately the same initial $x$ and $p$ values. This resembles a kind of period doubling. \Cref{fig:experiment:bifurcation} shows this kind of period doubling behaviour in a bifurcation diagram. 

From a $K = 0.8$ we can see that thee first mentionable filled regions start to grow in the upper and lower parts of the image in \cref{fig:experiment:fancy_k:8}. \todo[inline]{Close to the critical $K$?} Finally in the images shown in \cref{fig:experiment:fancy_k:20} - \ref{fig:experiment:fancy_k:40} we see that the orbits completely start to dominate the image. With in the last two images (\cref{fig:experiment:fancy_k:30} and \ref{fig:experiment:fancy_k:40}) only a few more stable initialisations left. 

\todo[inline]{Discuss similarities and differences to logistic map. Fuck}
